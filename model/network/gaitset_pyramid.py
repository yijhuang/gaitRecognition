...
class SetNet(nn.Module):
    def __init__(self, hidden_dim):
        ...
        self.set_layer2 = SetBlock(BasicConv2d(_set_channels[0], _set_channels[0], 3, padding=1), True, kernel_size=2)
        self.set_layer3 = SetBlock(BasicConv2d(_set_channels[0], _set_channels[1], 3, padding=1))
        self.set_layer4 = SetBlock(BasicConv2d(_set_channels[1], _set_channels[1], 3, padding=1), True, kernel_size=2)
        ...
        self.fc_bin = nn.ParameterList([
            nn.Parameter(
                nn.init.xavier_uniform_(
                    torch.zeros(sum(self.bin_num), 128+96+96, hidden_dim)))]) #nn.Parameter 将xxx转化成可训练的变量[62,128,256]
        
       # self.sub1_fc_bin = nn.ParameterList([
       #     nn.Parameter(
       #         nn.init.xavier_uniform_(
       #             torch.zeros(sum(self.bin_num), 96, hidden_dim)))])
       # self.sub2_fc_bin = nn.ParameterList([
       #     nn.Parameter(
       #         nn.init.xavier_uniform_(
       #             torch.zeros(sum(self.bin_num), 96, hidden_dim)))])
    ...
    def forward(self, silho, batch_frame=None):
        ...
        x = self.set_layer3(x)#self.frame_max[0]is data,self.frame_max[1] is indices
        ...
        #feature1 = feature1.matmul(self.fc_bin[0]) #（只是矩阵乘法）
        #feature2 = feature2.matmul(self.sub1_fc_bin[0])
        #feature3 = feature3.matmul(self.sub2_fc_bin[0])
