...
class SetNet(nn.Module):
    def __init__(self, hidden_dim):
        ...
        _gl_in_channels = 96
        _gl_channels = [64, 320, 128]
        self.gl_layer1 = BasicConv2d(_gl_in_channels, _gl_channels[0], 3, padding=1)
        self.gl_layer2 = BasicConv2d(_gl_channels[0], _gl_channels[1], 3, padding=1)
        self.gl_layer3 = BasicConv2d(_gl_channels[1], _gl_channels[1], 3, padding=1)
        self.gl_layer4 = BasicConv2d(_gl_channels[1], _gl_channels[2], 3, padding=1)
        ...
        self.fc_bin = nn.ParameterList([
            nn.Parameter(
                nn.init.xavier_uniform_(
                    torch.zeros(sum(self.bin_num) * 2, 128, hidden_dim)))]) #nn.Parameter 将xxx转化成可训练的变量[62,128,256]
    ...
    def forward(self, silho, batch_frame=None):
        ...
        gl = self.gl_layer1(self.frame_max(torch.cat([x2,x3],2))[0]+self.frame_mean(torch.cat([x2,x3],2)))
        gl = self.gl_layer2(gl)
        gl = self.gl_pooling(gl)
        ...
        gl = self.frame_max(torch.cat([x4,x5,x6],2))[0] +self.frame_mean(torch.cat([x4,x5,x6],2)) + gl
        gl = self.gl_layer3(gl)
        gl = self.gl_layer4(gl)
        #x = self.frame_max(x)[0] + self.frame_mean(x)[0]
        x = self.frame_max(x6)[0] + self.frame_mean(x6)
        gl = 0.1 * gl + x
