...
class SetNet3d(nn.Module):
    def __init__(self, hidden_dim, frame_num):
        ...
        self.layer2_3d = SetBlock3d(BasicConv3d(_set_channels[0], _set_channels[0], [1,3,3], padding=(0,1,1)),True, kernel_size=[2,2,2], stride=[2,2,2])
        ...
        self.layer4_3d = SetBlock3d(BasicConv3d(_set_channels[1], _set_channels[1],[1,3,3], padding=(0,1,1)),True, kernel_size=[2,2,2], stride=[2,2,2])
        ...
        self.layer6_3d = SetBlock3d(BasicConv3d(_set_channels[2], _set_channels[2],[1,3,3], padding=(0,1,1)),True, kernel_size=[2,2,2],stride=[2,2,2])
    ...
    def forward(self, silho, batch_frame=None):
        ...
        b, n, h, w = silho.size()
        n = silho.size(1) # n=128

        x_3d = silho.view(-1,self.frame_num,h,w).unsqueeze(1) #在siloho的dim=2增加一个维度=1 x.shape = [128,30,1,64,44] channels = 1
       ...
       #parameters 
        ...
        feature_3d = feature_3d.matmul(self.fc_bin[0])
        #feature_softmax = feature.matmul(self.fc_bin3[0])
        feature = feature_3d.permute(1,0,2).contiguous()
        _, n, d = feature.size()
