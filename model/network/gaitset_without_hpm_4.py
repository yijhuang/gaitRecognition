...
class SetNet(nn.Module):
    def __init__(self, hidden_dim):
        ...
        self.non_block1 = BasicConv2d(_set_channels[3], _set_channels[3], 1)
        self.non_block2 = BasicConv2d(_set_channels[3], _set_channels[3], 1)
        self.non_block3 = BasicConv2d(_set_channels[3], _set_channels[3], 1)
        self.non_block4 = BasicConv2d(_set_channels[3], _set_channels[3], 1)
    ...
    def frame_mean(self, x):
        if self.batch_frame is None:
            ...
            mean_list = torch.cat([_tmp[i][0] for i in range(len(_tmp))], 0)
    ...
    def forward(self, silho, batch_frame=None):
        ...
        b,c,h,w = x.shape #[96,320,16,11]
        #x_1 = self.non_block1(x)
        #x_2 = self.non_block2(x)

        x_1 = self.non_block1(x).view(b,c,-1)#(b,h*w,c)
        x_1 = x_1.permute(0,2,1)

        x_2 = self.non_block2(x).view(b,c,-1)#(b,c,h*w)
        x_3 = self.non_block3(x).view(b,c,-1).permute(0,2,1)#(b,h*w,c)
        x_4 = self.softmax(torch.matmul(x_1,x_2))
        # x_4 = self.softmax(x_1.matmul(x_2)).matmul(x_3).view(b,h,w,c)
        x_4 = torch.matmul(x_4,x_3).permute(0,2,1).contiguous()
        x_4 = self.non_block4(x_4.view(b,c,h,w))
        x = x + x_4 
        #gl = gl + x
